...
def backward(x,y,a):
    ...
    dz_dW = compute_dz_dW(x, Alen)
    dz_db = compute_dz_db(Alen)
...
def predict(Xtest, W, b):
    ...
    Q = Xtest.shape[0]
    ShapeW = W.shape[0]
    Y = np.zeros(Q) # initialize as all zeros
    P = np.asmatrix(np.zeros((Q,ShapeW)))
...
def check_dL_da(a, y, delta=1e-7):
    ...
    dL_da = np.asmatrix(np.zeros((c,1)))
    for i in range(c):
        d = np.asmatrix(np.zeros((c,1)))
        d[i] = delta
